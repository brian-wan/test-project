...
## Add/Subtract/Divide Two Numbers GUI

- Stack: Python 3 stdlib `tkinter` for the GUI, `pytest` for tests. No third-party GUI
  dependency.
- `add(a, b)`, `subtract(a, b)`, and `divide(a, b)` all live in `calculator.py` and
  share the same `entry_a`/`entry_b`/`result_label` widgets in the GUI via separate
  buttons (`on_add`/`on_subtract`/`on_divide` handlers). The Enter key shortcut stays
  bound to Add only.
